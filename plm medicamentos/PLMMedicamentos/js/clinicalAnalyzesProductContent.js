...
(function () {
    ...
    WinJS.UI.Pages.define("/pages/clinicalAnalyzes/clinicalAnalyzesProductContent.html", {
        ...
        ready: function (element, options) {
            ...
            function getProductDetail() {
                return new WinJS.Promise(function (completed, error, progress) {
                    Connectivity.test().then(function () {
                        var options = {
                            url: RESTServices.ClinicalAnalyzesEngine + '/getContentsByProduct?code=' + globalVars.code + "&ISBN=" + globalVars.clinicalAnalyzesISBN + "&productId=" + clinicalAnalyzesVars.productId,
                            type: 'GET'
                        };
            ...
            function displayProductContent(productDetails) {
                console.log(displayProductContent);
                var productContentElement = document.body.querySelector('#productContents');

                if (productDetails != null && productDetails != 'undefined' &&
                    productContentElement != null && productContentElement != undefined) {

                    var productNameElement =  document.body.querySelector("#productName");
                    ...
                    if (productDetails.HTMLContent != '') {
                        productContentElement.innerHTML = productDetails.HTMLContent;
                    } else {
                        productContentElement.innerHTML = '<p class="alignCenter">Detalle del producto no disponible.</p>';
                    }
                } else {
                    productContentElement.innerHTML = '<p class="alignCenter">Detalle del producto no disponible.</p>';
                }
